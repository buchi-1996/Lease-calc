...
class Calculator {
    ...
    calculateResult = () => {
        if(+this.downPayment.value.trim().replace(/,/g, '')  >= +this.msrp.value.trim().replace(/,/g, '') ) {
            return 0
        }

        return Math.round(((this.msrp.value.trim().replace(/,/g, '')  - this.downPayment.value.trim().replace(/,/g, '') ) - this.tradeInValue.value.trim().replace(/,/g, '')  ) / this.selectedMonth ) || 0
    ...
    updateResult = (e) => {
        ...
        let newNum = num.replace(/[^0-9,]/g, '').split(/(?=(?:\d{3})+$)/).join(",");
    ...
    showTerms = () => {
        this.terms.innerHTML = this.months.map(month => {
            return (`<li class='list-item ${this.selectedMonth === month && 'active'}' data-month=${month} >${month} mo.</li>`)
        }).join('')
    ...
    <li>
        ...
        <span>$${(Math.round((this.msrp.value.trim().replace(/,/g, '')  - this.downPayment.value.trim().replace(/,/g, '') ) - this.tradeInValue.value.trim().replace(/,/g, '')  )).toLocaleString('en-US')}</span>
    </li>
    <li>
